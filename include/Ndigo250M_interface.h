 ...
 *	
 *	Unlike the Ndigo5G-10, this device has an individual DMA buffer for each data source.
 */
...
typedef struct  {
	...
	/*! \brief Enable capturing of TDC measurements on external digital input channel.
	 *
	 *	Only possible for divisor=2 for the Ndigo250M-14 and not possilbe at all for the
	 *	Ndigo125M. For lower sampling rates the decimation option can be used.
	 */
	 ...
	 *
	 *	Threshold is ignored for entries 8 and above. The trigger indices refer to the entry in the
	 *	trigger array and are defined like NDIGO_TRIGGER_*. Sources 16 to 21 are positive edge
	 *	triggered.
	 */
	 ...
	 *
	 *	A structure describing the gating blocks that can be used by the trigger blocks to filter
	 *	triggers.
	 */
	 ...
	 *
	 *	Each integer represents a bitmask selecting the trigger sources for that line. The bit 
	 *	mapping is described in the structure ndigo_trigger_block.
	 */
	int drive_bus[4];
	 ...
	 *
	 *	The 500MHz base clock is divided by this divisor to create the rsampling frequency. Valid
	 *	values are 2 to 32 for the Ndigo250M and 4 to 32 for the Ndigo125M.
	 */
	int divisor;
	 ...
	 *
	 *	The value can be set separately for the two ADCs. Gain[0] modifies channels 0 and 1,
	 *	gain[1] modifies channels 2 and 3. Default is 0.
	 */
	int high_gain[2];
 ...
 */
NDIGO_API int ndigo250m_get_default_configuration(ndigo_device *device,ndigo250m_configuration *config);

/*!	\ingroup conffuncts
 *	\brief	Copies the current configuration to the specified config pointer
 */
NDIGO_API int ndigo250m_get_current_configuration(ndigo_device *device, ndigo250m_configuration *config);
 ...
 */
NDIGO_API int ndigo250m_configure(ndigo_device *device, ndigo250m_configuration * config);

/*! \ingroup initfuncts
 *	\brief returns net number of boards that are supported by this driver in the system.
 */
NDIGO_API int ndigo250m_count_devices(int *error_code, const char** error_message);
 ...
 */
NDIGO_API int ndigo250m_get_default_init_parameters(ndigo_init_parameters *init);

/*!	\ingroup initfuncts
 *	\brief Open and initialize the Ndigo250M-14 board with the given index.
 *
 *	With error_code and error_message the user must provide pointers where to buffers where
 *	error information should be written by the driver. The buffer for the error message must
 *	by at least 80 chars long.
 *	
 *	Params is a structure of type ndigo_init_parameters that must be completely
 *	initialized.
 */
NDIGO_API ndigo_device *ndigo250m_init(ndigo_init_parameters *params,int *error_code, const char** error_message);
 ...
 *
 *	For each DMA channel returns a pointer to an array of captured data in read_out. The result can
 *	contain any number of packets of type ndigo_packet. read_in provides parameters to the driver.
 *	A call to this method automatically allows the driver to reuse the memory returned in the
 *	previous call.
 *
 *	Returns an error_code as definded in the structure ndigo_read_out.
 */
NDIGO_API int ndigo250m_read(ndigo_device *device, ndigo250m_read_in *in, ndigo250m_read_out* out);

/*! \ingroup readfuncts
 *	\brief Acknowledge all data up to the packet provided as parameter.
 *
 *	This is mandatory if acknowledge_last_read in the ndigo_read_in structure is set to false for
 *	calls to ndigo_read.
 *
 *	This feature allows to either free up partial DMA space early if there will be no call to
 *	ndigo_read anytime soon. It also allows to keep data over multiple calls to ndigo_read to
 *	avoid unnecessary copying of data.
 */
NDIGO_API int ndigo250m_acknowledge(ndigo_device *device, int channel, volatile ndigo_packet *packet);
