 ...
 *	\brief bit mask with a bit set for attribute sources in @link
 *ndigo_trigger_block ndigo_trigger_block @endlink and @link ndigo_gating_block
 *ndigo_gating_block @endlink
 *
 *	also used by @link ndigogat ndigo_gating_block @endlink
 */
/*! \defgroup gatedefs #defines for gates
 *	\brief bit mask with a bit set for all trigger @link ndigo_trigger_block
 *gates @endlink
 */
/*!@}*/
 ...
 *
 * This structure configures the merging of the inputs from the synchronization
 * bus and the optional extension board.
 */
/*!@}*/
 ...
 *
 *	There are six LEDs on the front panel. The intensity of the red and green
 *part can be set from 0 to 255. There is no blue component in the current
 *version. Per default all LEDs are set to auto mode. This means that used
 *channels are lit green, activity is shown as yellow and overflow is shown as
 *red.
 */
/*!@}*/
...
#define NDIGO_ADD_TRIGGER_COUNT                                                \
    6 //!< additional set of triggers for digital inputs
/*!@}*/
...
#define NDIGO_INVALID_DEVICE                                                   \
    5 //!< The pointer given to a Ndigo driver function was not a valid pointer
...
struct ndigo_packet {
     ...
     * Defaults to 0 if no value is assigned to the parameter
     * @link initparamsstruct board_id in Structure ndigo_init_parameters
     * @endlink or set via @link conffuncts int ndigo_set_board_id(ndigo_device
     * * device, int board_id) @endlink
     */
    unsigned char card;
     ...
     *	For the TDC channel it is set to 8 to signify 64bit unsigned data - only
     *for internal processing. If the type field is 128 or greater then there is
     *no data present, even if length is not 0. In this cases the length field
    ...
    /*! \brief Number of 64-bit elements (each containing 4 samples) in the data
     *array < 128.
     *
     ...
     *trigger sources, i.e. bit 0 is set if trigger A0 was active, bit 29 is set
     *if trigger @link sourcedefs BUS3_PE @endlink was active. Use the
     *	@link sourcedefs NDIGO_TRIGGER_SOURCE_* @endlink defines to check for
     *the bits set.
     */
    unsigned int length;
     ...
     *	TDC: timestamp of the trigger event (falling edge) of the TDC channel.
     *When
     *	@link readfuncts ndigo_process_tdc_packet()@endlink is called once on
     *the packet the timestamp is replaced with the precise timestamp for the
     *edge. Timestamp channel: timestamp of the trigger event.
     */
    uint64_t timestamp;

    /*! \brief sample data
     *
     * For the Ndigo5G each 64 bit word vontains four 16bit signed words from
     * the ADC. The user can cast the array to short* to directly operate on the
     * sample data.
     */
    uint64_t data[1];
};
...
typedef struct {
     ...
     * of -32768 and +32768.
     *
     * For trigger indices @link triggerdefs NDIGO_TRIGGER_TDC @endlink to @link
     * triggerdefs NDIGO_TRIGGER_BUS3_PE @endlink the threshold is ignored.
     *
     *  Valid values depend on output mode:
     *  - @link outputdefs NDIGO_OUTPUT_MODE_RAW @endlink : 0 to 1023
     *  - @link outputdefs NDIGO_OUTPUT_MODE_SIGNED16 @endlink : -32768 to
     * +32767
     */
    short threshold;
     ...
     *	- 1: edge trigger
     *
     * For trigger indices @link triggerdefs NDIGO_TRIGGER_AUTO @endlink and
     *@link triggerdefs NDIGO_TRIGGER_ONE @endlink this is ignored For trigger
     *indices @link triggerdefs NDIGO_TRIGGER_TDC_PE @endlink to @link
     *triggerdefs NDIGO_TRIGGER_BUS3_PE @endlink this must be set.
     *
     ...
     *
     * For trigger indices @link triggerdefs NDIGO_TRIGGER_AUTO @endlink and
     * @link triggerdefs NDIOG_TRIGGER_ONE @endlink this is ignored For trigger
     * indices @link triggerdefs NDIGO_TRIGGER_TDC_PE @endlink to @link
     * triggerdefs NDIGO_TRIGGER_BUS3_PE @endlink this must be set.
     */
    ndigo_bool_t rising;
...
typedef struct {
     ...
     * similar. Set to 0 for all versions up to first release. Must be set to
     * @link defversion NDIGO_API_VERSION @endlink
     */
    int version;
     ...
     * driver as reported by ndigo count devices. This index selects one of
     * them. Boards are enumerated depending on the PCIe slot. The lower the bus
     * number and the lower the slot number the lower the card index.
     */
    int card_index;

    /*! \brief This 8bit number is filled into each packet created by the board
     *
     * and is useful if data streams of	multiple boards will be merged. If only
     * Ndigo250M-14 cards are used this number can be set to the card index. If
     * boards of different types that use a compatible data format are used in a
     * system each board should get a unique id. Can be changed with int
     * @link conffuncts ndigo_set_board_id(ndigo_device *device, int board_id)
     * @endlink
     */
    int board_id;
    ...
    /*! \brief If >0 the direct sync method is used for multi board
     * applications.
     *
     * Value corresponds to accuracy (higher values for more accurate phase
     * match.
     */
    int direct_sync;
     ...
     * Ndigo5G and Ndigo250M boards are present in the system. If other boards
     * are synchronized the correct value is the lowest common denominator of
     * the value of all boards.
     */
    int sync_period;
     ...
     *
     * If true/1, a valid data window is automatically detected at
     * initialization. Default value is false/0: values from flash memory are
     * used in order to set the data window to correct position
     */
    ndigo_bool_t force_window_calibration;

    /*! \brief enables/disables	the CLK and sync line for the HPTDC
     *
     *	Must be set to false. If an HPTDC is connected to the board (valid for
     *Ndigo5G) this enables/disables the CLK and sync line for the HPTDC
     */
     ...
     * buffer_size[0]. Ndigo250M uses 6 entries.
     */
     ...
     *
     * the buffers will be allocated with the sizes given above, make sure that
     * the memory is large enough. For NDIGO_BUFFER_USE_PHYSICAL, the address of
     * the preallocated buffer. Ignored. Might be used for future buffer types.
     */
    ...
    /*! \brief Can be used to activate future device variants such as different
     * base frequencies.
     *
     * Set to 0.
     */
     ...
     * CRONO_DEVICE_*.
     *
     * Initialized by @link initfuncts ndigo_get_default_init_parameters()
     * @endlink. Must be left unchanged.
     */
    int device_type;
     ...
     *
     * Initialized by @link initfuncts ndigo_get_default_init_parameters()
     * @endlink. The write pointer updated is delayed by this number of 4n clock
...
typedef struct {
    ...
    ndigo_bool_t force_led; //!< Activates LED on input even if trigger block is
                            //!< not enabled. Required for LED activity in
                            //!< Multi/SingleShot mode.
     ...
     * trigger. The retrigger setting is ignored for the timestamp channel.
     */
    ...
    /*! \brief Precursor in multiples of the device main clk (Ndigo5G: 3.2ns,
     * Ndigo250M & Ndigo125M: 4 ns). The amount of data preceding a trigger that
     * is captured. The	precursor setting is ignored for the timestamp channel.
     * Maximum is set by @link precursordef NDIGO_MAX_PRECURSOR @endlink.
     */
    int precursor;

    /*! \brief Length in multiples of the device main clk (Ndigo5G: 3.2ns,
     * Ndigo250M & Ndigo125M: 4 ns).
     *
     ...
     * trigger window always one main clk cycle wide (Ndigo5G: 3.2ns, Ndigo250M
     * & Ndigo125M: 4 ns), in level trigger mode it is as long as the trigger
     * condition is fulfilled. The length setting is ignored for the timestamp
     * channel.
     */
    int length;
     ...
     *
     * Default @link sourcedefs NDIGO_TRIGGER_SOURCE_x @endlink (x = 0 for index
     * 0 etc)
     */
    int sources;
...
typedef struct {
     ...
     *
     * Defaults to true. If set, a gate is created with the set timing from the
     * first occurrence of the input trigger even for short gates. If not set,
     * the input signal must persist for the gate to be created.
     */
    ndigo_bool_t extend;
    ndigo_bool_t reserved1; //!< Defaults to false. Do not change.
     ...
     *
     * In multiples of 3.2ns. Start needs to be >=0 and <2^16, start must be set
     * to a value <=stop.
     */
    int start;
     ...
     * stop time. In multiples of 3.2ns. Stop needs to be >=0 and <2^16.
     */
    int stop;
     ...
     *
     * The gates cannot use the additional digital trigger sources @link
     * sourcedefs NDIGO_TRIGGER_SOURCE_TDC_PE  @endlink to
     * @link sourcedefs NDIGO_TRIGGER_SOURCE_BUS3_PE @endlink. Default is @link
     * sourcedefs NDIGO_TRIGGER_SOURCE_x0 @endlink (0 for index 0 etc).
     */
    int sources;
...
typedef struct {
     ...
     *
     * If false input signal and BUS signal are read before routing to the
...
/*! \ingroup runtime
 * \brief Call this to resume data acquisition after a call to ndigo pause
 * capture.
 */
 ...
 *
 * This is mandatory if acknowledge last read in the ndigo read in structure is
 * set to false for calls to ndigo read.
 */
...
struct ndigo_static_info {
     ...
     *
     * or set via int @link conffuncts ndigo_set_board_id(ndigo_device *device,
     * int board_id) @endlink.
     */
    int board_id;
     ...
     * numbers. * E.g. 0x010103 codes version 1.1.3. A change in the first digit
     * generally requires a recompilation of user applications. Change in the
     ...
     *
     * This can be read from a register. It is a four bit number that changes
     ...
     *		- bit 0 determines whether this board contains an 8-bit or 10-bit
     *ADC
     ...
     *		- bit 2 determines input connectors (0: single ende, 1:
     *differential)
    ...
    /*! \brief chipID as read from the 16 bit adc chip id register at SPI
     * address 0.
     *
     ...
     *
     * The number is identical to the one printed on the silvery sticker on the
     * board.
     */
    int board_serial;
    ...
    /*! \brief If not 0 the driver found valid calibration data in the flash on
     ...
     *
     * DIN EN ISO 8601 string YYYY-MM-DD HH:DD describing the time when the card
     * was calibrated.
     */
     ...
     *
     * This can be read from a register since board rev. 7. It is a four bit
     * number that changes when the schematic of the FPGA module is changed.
     */
    int fpga_module_revision;

    /*! \brief The same FPGA module can be populated in multiple variants.
     *
     * This is a six bit code that can be read from a register since board
     * rev. 7.
     */
...
/*! \ingroup statfuncts
 * \brief contains the information about the board which does not change during
 * run time.
 */
...
typedef struct {
    ...
    /*! \brief Bandwidth.
     *
     * Currently fixed at 3G, configurable to 1G (fullBW=false) for 10bit
     * version.
     */
    double bandwidth;
...
/*! \ingroup statfuncts
 * \brief returns information that changes indirectly due to configuration
 * changes.
 */
...
/*! \ingroup statfuncts
 * \brief returns dynamic information that can be obtained within a few
 * microseconds.
 */
 ...
 *ndigo_static_info::driver_revision
 */
 ...
 *accurate value. If called more than once on a packet the timestamp will be
 *invalid.
 */
 ...
 *
 * can be changed after initialization of the card. If cronotools are used the
