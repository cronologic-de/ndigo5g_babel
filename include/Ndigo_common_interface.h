...
#define ndigo_next_packet(current) ((volatile ndigo_packet*) (((__int64) (current)) +( ((current)->type&128?0:(current)->length) + 2) * 8))
...
typedef struct {
	 ...
	 * future buffer types.
	 */
	unsigned __int64 buffer_address;
...
typedef struct {
	...
	/*! \brief Precursor in multiples of the device main clk (Ndigo5G: 3.2ns, Ndigo250M & Ndigo125M: 4 ns).
	...
	/*! \brief Length in multiples of the device main clk (Ndigo5G: 3.2ns, Ndigo250M & Ndigo125M: 4 ns).
	 *
	 ...
	 * trigger mode the trigger window always one main clk cycle wide (Ndigo5G: 3.2ns, Ndigo250M & Ndigo125M: 4 ns), 
	 * in level trigger mode it is as long as the trigger condition is fulfilled. The length setting 
	 * is ignored for the timestamp channel.
	 */
	int length;
 ...
 * ndigo read.
 */
NDIGO_API int ndigo_acknowledge(ndigo_device *device, volatile ndigo_packet *packet);

/**
* Fixed length of calibration date string.
* calibration date format: YYYY-MM-DD hh:mm
*/
static const int NDIGO_CALIBRATION_DATE_LEN = 20;
...
/*!	\ingroup statfuncts
*	\brief Returns the driver version, same format as ndigo_static_info::driver_revision
*/
 ...
 */
NDIGO_API int ndigo_process_tdc_packet(ndigo_device* device, volatile ndigo_packet * packet);
