 ...
 *
 * Mapping has to be monotonous. It can be used e.g. for custom INL, offset and
 *gain correction that covers user front end electronics. It can also invert the
 *signal or correct the effect of logarithmic input amplifiers etc. The LUT is
 *applied on the board, thus using it does not cause any additional CPU load. In
 *the mode @link outputdefs NDIGO_OUTPUT_MODE_CUSTOM_INL @endlink the on-board
 *INL correctin table is applied before the user function, while @link
 *outputdefs NDIGO_OUTPUT_MODE_CUSTOM @endlink does not perform INL correction.
 *In order to use the user lookup table functionality lut_func must be set to a
 ...
 * conjunction with @link conffuncts ndigo_get_default_configuration() @endlink,
 *@link conffuncts ndigo_get_current_configuration() @endlink and @link
 *conffuncts ndigo_configure() @endlink. It uses internally the structures @link
 *ndigotrgblock ndigo_trigger_block @endlink and
...
typedef struct {
     ...
     *
     * Set to the minimum bandwidth required for the application. Lower
     * bandwidth results in reduced noise. The driver will set the ADC to the
     * minimum setting that has at least the desired bandwidth and report the
     * selected bandwidth in the ndigo param info structure. The -8, -10 and -12
     * versions currently supports 1GHz and 3GHz bandwidth, the -8AQ version
     * supports 2GHz, 1.5GHz, 600MHz and 500 MHz.
     */
    double bandwidth;
    ...
    /* Set DAC on channels 5 (GATE) and 6 (TRIG). Default offset for both is
     * -0.35V. */
     ...
     * For negative 0.8 V NIM pulses a value of -0.35 should be set.
     */
    double dc_offset[2];
     ...
     * to the entry in the trigger array @link triggerdefs NDIGO_TRIGGER_*
     * @endlink
     */
     ...
     *
     * A structure describing the routing of the 4 digital channels of the Ndigo
     * extension board to the trigger matrix.
     */
    ndigo_extension_block extension_block[NDIGO_EXTENSION_COUNT];

    /*! \brief Enables output drives.
     *
     * Enables output drive for each of the four external sync lines. Each
     * integer represents a bitmask selecting the trigger sources for that line.
     * The bit mapping is described in @link ndigotrgblock ndigo_trigger_block
     * @endlink.
     */
    int drive_bus[4];
     ...
     *
     * Signed16 scales and INL-corrects the input. RAW directly presents the ADC
     * values. Defines can be found in @link outputdefs NDIGO_OUTPUT_MODE_*
     * @endlink
     */
    int output_mode;
     ...
     *
     * If the output_mode is set to @link outputdefs NDIGO_OUTPUT_MODE_CUSTOM
     * @endlink or @link outputdefs NDIGO_OUTPUT_MODE_CUSTOM_INL @endlink this
     * function is used for mapping from ADC value to output value. The driver
     * will call this function with a value from -1 to +1 and the function must
     * return the corresponding signed 16 bit value that the board should return
     * for an input voltage relative to the full scale range.
     */
    lut_func custom_lut;
...
/*! \ingroup initfuncts
 * \brief returns the number of boards in the system that are supported by this
 *driver \param *error_code is type int \param **error_message is type const
 *char
 */
NDIGO_API int ndigo_count_devices(int *error_code, const char **error_message);

/*! \ingroup initfuncts
 * \brief get a set of default parameteres to feed into @link initfuncts
 *ndigo_init()@endlink.
 *
 * This must always be used to initialize the @link initparamsstruct
 *ndigo_init_parameter structure @endlink. \param *init is type @link
 *initparamsstruct ndigo_init_parameter @endlink
 */
 ...
 *
 * With error_code and error_message the user must provide pointers where error
 *information should be written by the driver. The buffer for the error message
 *must contain at least 80 chars. \param *params is type @link initparamsstruct
 *ndigo_init_parameters @endlink \param *error_code is type int \param
 ***error_message is type char. The buffer for the error message has to contain
 *at least 80 chars.
 */
...
struct ndigo_read_in {
     ...
     *
     * if set, @link readfuncts ndigo_read() @endlink automatically acknowledges
...
struct ndigo_read_out {
     ...
     * @link readfuncts ndigo_read() @endlink.
     */
 ...
 * \brief reads packets
 *
 * Returns a pointer to an array of captured data in @link readout read_out
 * @endlink. The result contains any number of packets of type @link ndigopack
 * ndigo_packet @endlink. @link readin read_in @endlink provides parameteres to
 * the driver. A call to this method automatically allows the driver to reuse
 * the memory returned in the previous call.
 *
 * Returns an error code as defined in the structure @link readout
 * ndigo_read_out @endlink
 */
 ...
 * mandatory if @link readin acknowledge_last_read @endlink in the @link readin
 * ndigo_read_in structure @endlink is set to false for calls to @link
 * readfuncts ndigo_read() @endlink.
 *
...
/*! \ingroup conffuncts
 *	\brief converts output value from one @link outputdefs output_mode @endlink
 *to another @link outputdefs output_mode @endlink.
 */
