...
/* A function that maps a value between 0 and 1 to a 16-bit output value ), must be monotonous */
...
typedef struct  {
	...
	/* Sets the offset-values to dac_neutral +- this value in volts. 
	   (Programmed parameter is dac_neutral +- (channel_offset*64k/2.5). Defaults to 0.
	*/
	...
	/* component to create a trigger either periodically or randomly.
	   To be exact, there are two parameters M and N that result in a distance between triggers of
	   T = 1 + M + [1...2^N]
	   clock cycles.
	   0 <= M < 2^32
	   0 <= N < 32
	...
	*/
	...
	int output_mode;
	/* if the output_mode was set to custom or custom_inl  this func is used for mapping from ADC value to output value*/
	lut_func custom_lut;
} ndigo_avrg_configuration;

/* Copies the default configuration to the specified config pointer*/
NDIGO_API int ndigo_avrg_get_default_configuration(ndigo_device *device, ndigo_avrg_configuration *config);

/* Copies the current configuration to the specified config pointer*/
NDIGO_API int ndigo_avrg_get_current_configuration(ndigo_device *device, ndigo_avrg_configuration *config);

/* Configures Ndigo device, the config information is copied, so can be changed after the called*/
NDIGO_API int ndigo_avrg_configure(ndigo_device *device, ndigo_avrg_configuration * config);

/* get the number of boards)*/
NDIGO_API int ndigo_avrg_count_devices(int *error_code, const char** error_message);

/* Sets up the standard parameters*/
NDIGO_API int ndigo_avrg_get_default_init_parameters(ndigo_init_parameters *init);


/* init the given card for the given client  version number, use NDIGO_API_VERSION)*/
NDIGO_API ndigo_device *ndigo_avrg_init(ndigo_init_parameters *params, int *error_code, const char** error_message);

/* The parameters of the read commands*/
struct ndigo_avrg_read_in {	
	ndigo_bool_t acknowledge_last_read;  //if set, ndigo_read automatically acknowledges packets from the last read
};
...
/** Reads packets from the board, optionally automatically acknowledges the last read packets*/
NDIGO_API int ndigo_avrg_read(ndigo_device *device, ndigo_avrg_read_in *in, ndigo_avrg_read_out* out);

/* this method acknowledges the processing of data up to and including the packet passed as parameter */
NDIGO_API int ndigo_avrg_acknowledge(ndigo_device *device, volatile ndigo_packet *packet);

NDIGO_API short ndigo_avrg_convert_value(ndigo_device* device, int channel, short val, int outputModeFrom, int outputModeTo);
